...
#Questions
#1. Why is this a supervised task? It is supervised because we gave it the prior training data with the correct labels,
#so it can learn and predict on the new data.
#2. Would adding a day of the week parameter improve the model? I dont think it would be a strong enough factor to determine tennis playing 
#because days of the week hold no bearing on weather conditions, if thats what we base our decision on.
#3. A very deep tree would have more branches with only a few parameters to test, leading to overfitting.
#4. Another tree is a tree to get ice cream. The features would be weather, temperature, and time of day. the label would be whether or no to get ice cream.
